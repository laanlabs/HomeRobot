...
class RadialSliderView : UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let path3 = UIBezierPath(rect: CGRect.init(x: 0, y: bounds.size.height * 0.5 - 5,
                                                   width: bounds.size.width, height: 10))
        UIColor.white.withAlphaComponent(0.3).setFill()
        path3.fill()
        
        // Thumb
        let pos_x : CGFloat = w * (CGFloat(self.value) + 1.0) * 0.5
        let thumbSize : CGFloat = 35.0
        
        let thumbRect = CGRect.init(origin: CGPoint.init(x: pos_x - thumbSize * 0.5, y: bounds.size.height * 0.5 - thumbSize * 0.5),
...
class TouchDriveView {
    ...
    init(size : CGFloat) {
        ...
        powerView.frame = CGRect.init(x: 0, y: 0, width: size, height: size)
        
        powerView.backgroundColor = UIColor.clear
        powerView.transform = CGAffineTransform.init(rotationAngle: -CGFloat.pi * 0.5 )
        
        steeringView.frame = CGRect.init(x: 0, y: 0, width: size, height: size)
...
//        self.addSubview(steeringView)
    ...
    @objc func handlePan( _ pan : UIPanGestureRecognizer ) {
        ...
        self.delegate?.valueChanged(steering: steeringView.value, power: powerView.value)
