...
public extension UIImage {
    ...
    /**
     ...
     - Returns A new image
     */
    convenience init?(text: String, font: UIFont = UIFont.systemFont(ofSize: 18), color: UIColor = UIColor.white, backgroundColor: UIColor = UIColor.gray, size: CGSize = CGSize(width: 100, height: 100), offset _: CGPoint = CGPoint(x: 0, y: 0)) {
    ...
    func apply(padding: CGFloat) -> UIImage? {
        ...
        context?.draw(cgImage!, in: imageLocation)
    ...
    func crop(bounds: CGRect) -> UIImage? {
        return UIImage(cgImage: (cgImage?.cropping(to: bounds)!)!,
                       scale: 0.0, orientation: imageOrientation)
    }
    ...
    func getSquareInset() -> CGRect? {
        let maxSize = max(size.height, size.width)

        let dx = CGFloat((maxSize - size.width) / 2)
        let dy = CGFloat((maxSize - size.height) / 2)

        let insetRect: CGRect = CGRect(x: dx, y: dy, width: size.width, height: size.height)

        return insetRect
    }

    func fillToSquare() -> UIImage? {
    ...
    class func image(fromURL url: String, placeholder: UIImage, shouldCacheImage: Bool = true, closure: @escaping (_ image: UIImage?) -> Void) -> UIImage? {
        ...
        if let nsURL = URL(string: url) {
            session.dataTask(with: nsURL, completionHandler: { (data, _, error) -> Void in
                if error != nil {
